...
<html lang="zh-cn">
  ...
  <table border="0.1" align="center" class="content1">
    <c:forEach var="query" items="${sessionScope.borrowquery }">
    	<tr>
    		<td><img src="${query.getB_img()}" width="80px" height="80px"/></td>
    		<td>书名：${query.getB_name()}</td>
    		<td>${query.getB_author()} 著</td>
    		<td>${query.getB_press()} 出版</td>
    		<td>剩余：${query.getB_stock()} 本</td>
    		<td><a href="borrow.view?id=${query.getB_id()}">借书</a></td>
    	</tr>
    </c:forEach>
